/**
 * Filters an array of objects, returning rows where any specified key's value includes with the filterValue (case-insensitive).
 *
 * @template T
 * @param {T[]} data - The data array to filter
 * @param {string} filterValue - The value to match if Included in any of selected key's value
 * @param {(keyof T)[]} keys - The property keys to search on
 * @returns {T[]} - Filtered array of objects
 */
export function includesSearch<T extends Record<string, any>>(
    data: T[],
    filterValue: string,
    keys: string[]
): T[] {
